...
void TossClientItems( gentity_t *self ) {
	...
	if (g_instantgib.integer || g_rockets.integer || g_gametype.integer == GT_CTF_ELIMINATION){
	//Nothing!	
	}
	else
	if ( weapon > WP_MACHINEGUN && weapon != WP_GRAPPLING_HOOK && 
		...
		if(om_frag_drop_weapon.integer==1){
	...
	if ( g_gametype.integer != GT_TEAM ) {
		...
		for ( i = 1 ; i < PW_NUM_POWERUPS ; i++ ) {
			if ( self->client->ps.powerups[ i ] > level.time ) {
				...
				if(om_frag_drop_powerup.integer==1){
...
int G_LocationDamage(vec3_t point, gentity_t* targ, gentity_t* attacker, int take) {
	...
	// The upper body never changes height, just distance from the feet
		...
		{
		case LOCATION_HEAD:
			take *= om_location_head.value;
			break;
		case LOCATION_FACE:
			if (targ->client->lasthurt_location & LOCATION_FRONT)
				take *= om_location_face.value; // Faceshots REALLY suck
			else
				take *= om_location_head.value;
			break;
		case LOCATION_SHOULDER:
			if (targ->client->lasthurt_location & (LOCATION_FRONT | LOCATION_BACK))
				take *= om_location_throat.value; // Throat or nape of neck
			else
				take *= om_location_shoulders.value; // Shoulders
			break;
		case LOCATION_CHEST:
			if (targ->client->lasthurt_location & (LOCATION_FRONT | LOCATION_BACK))
				take *= om_location_torso.value; // Belly or back
			else
				take *= om_location_arms.value; // Arms
			break;
		case LOCATION_STOMACH:
			take *= om_location_stomach.value;
			break;
		case LOCATION_GROIN:
			if (targ->client->lasthurt_location & LOCATION_FRONT)
				take *= om_location_groin.value; // Groin shot
			break;
		case LOCATION_LEG:
			take *= om_location_legs.value;
			break;
		case LOCATION_FOOT:
			take *= om_location_feet.value;
			break;
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	// damage multiplier
	damage *= om_damage_multiplier.value;
	...
	if ( targ->client && mod != MOD_JUICED) {
        ...
        if( (!attacker || attacker->s.eType != ET_PLAYER) && client->lastSentFlying>-1 && ( mod==MOD_FALLING || mod==MOD_LAVA || mod==MOD_SLIME || mod==MOD_TRIGGER_HURT ) )  {
            if(client->lastSentFlyingTime+4000<level.time) {
                client->lastSentFlying = -1; //More than 4 seconds, not a kill!
            } else {
...
//#endif
	...
	knockback *= om_knockback_multiplier.value;

	if ( knockback > om_knockback_maximum.integer ) {
		knockback = om_knockback_maximum.integer;
	}

	if ( knockback < om_knockback_minimum.integer ) {
		knockback = om_knockback_minimum.integer;
	}
	...
	if(om_no_knockback.integer==1){ knockback = 0; }
	...
	if ( knockback && targ->client ) {
		...
		//mass = 200;
		mass = om_knockback_mass.integer;
		...
		if ( !targ->client->ps.pm_time ) {
			...
			//	t = 200;
			//}
			if ( t > om_knockback_maximum.integer ) {
				t = om_knockback_maximum.integer;
			}
...
//#endif
	...
	if ( targ == attacker) {
		// oatmeal begin
		//damage *= 0.5;
		damage *= om_self_damage_multiplier.value;
		// oatmeal end
	}
...
//#endif
	...
	if (targ->client) {
		...
		if(om_locational_damage.integer==1){
	...
	if(mod==MOD_MACHINEGUN){
		if(om_machinegun_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
				}
			}
		}
	}
	...
	if(mod==MOD_SHOTGUN){
		if(om_shotgun_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if(mod==MOD_RAILGUN){
		if(om_railgun_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if(mod==MOD_LIGHTNING){
		if(om_lightning_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if(mod==MOD_NAIL){
		if(om_nailgun_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if(mod==MOD_CHAINGUN){
		if(om_chaingun_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if(mod==MOD_PROXIMITY_MINE){
		if(om_proxmine_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if((mod==MOD_GRENADE)||(mod==MOD_GRENADE_SPLASH)){
		if(om_grenade_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if((mod==MOD_ROCKET)||(mod==MOD_ROCKET_SPLASH)){
		if(om_rocket_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
	...
	if((mod==MOD_PLASMA)||(mod==MOD_PLASMA_SPLASH)){
		if(om_plasma_vampire.integer==1){
			if(attacker->health<om_vampire_maximum.integer){
				attacker->health += take;
				if(attacker->health>om_vampire_maximum.integer){
					attacker->health = om_vampire_maximum.integer;
