...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	// Infinite Ammo
	if(om_rocket_unlimited_ammo.integer==1){ client->ps.ammo[WP_ROCKET_LAUNCHER] = 999; }
	if(om_grenade_unlimited_ammo.integer==1){ client->ps.ammo[WP_GRENADE_LAUNCHER] = 999; }
	if(om_plasma_unlimited_ammo.integer==1){ client->ps.ammo[WP_PLASMAGUN] = 999; }
	if(om_machinegun_unlimited_ammo.integer==1){ client->ps.ammo[WP_MACHINEGUN] = 999; }
	if(om_shotgun_unlimited_ammo.integer==1){ client->ps.ammo[WP_SHOTGUN] = 999; }
	if(om_bfg_unlimited_ammo.integer==1){ client->ps.ammo[WP_BFG] = 999; }
	if(om_railgun_unlimited_ammo.integer==1){ client->ps.ammo[WP_RAILGUN] = 999; }
	if(om_chaingun_unlimited_ammo.integer==1){ client->ps.ammo[WP_CHAINGUN] = 999; }
	if(om_lightning_unlimited_ammo.integer==1){ client->ps.ammo[WP_LIGHTNING] = 999; }
	if(om_nailgun_unlimited_ammo.integer==1){ client->ps.ammo[WP_NAILGUN] = 999; }
	if(om_proxmine_unlimited_ammo.integer==1){ client->ps.ammo[WP_PROX_LAUNCHER] = 999; }

	// Set STAT_ROF to the right rate-of-fire depending on
	// the current weapon
	switch( client->ps.weapon ) {
		case WP_GAUNTLET:
			client->ps.stats[STAT_ROF] = om_gauntlet_rof.integer;
			break;
		case WP_GRAPPLING_HOOK:
			client->ps.stats[STAT_ROF] = om_grappling_hook_rof.integer;
			break;
		case WP_MACHINEGUN:
			client->ps.stats[STAT_ROF] = om_machinegun_rof.integer;
			break;
		case WP_SHOTGUN:
			client->ps.stats[STAT_ROF] = om_shotgun_rof.integer;
			break;
		case WP_PLASMAGUN:
			client->ps.stats[STAT_ROF] = om_plasma_rof.integer;
			break;
		case WP_GRENADE_LAUNCHER:
			client->ps.stats[STAT_ROF] = om_grenade_rof.integer;
			break;
		case WP_ROCKET_LAUNCHER:
			client->ps.stats[STAT_ROF] = om_rocket_rof.integer;
			break;
		case WP_RAILGUN:
			client->ps.stats[STAT_ROF] = om_railgun_rof.integer;
			break;
		case WP_NAILGUN:
			client->ps.stats[STAT_ROF] = om_nailgun_rof.integer;
			break;
		case WP_BFG:
			client->ps.stats[STAT_ROF] = om_bfg_rof.integer;
			break;
		case WP_CHAINGUN:
			client->ps.stats[STAT_ROF] = om_chaingun_rof.integer;
			break;
		case WP_LIGHTNING:
			client->ps.stats[STAT_ROF] = om_lightning_rof.integer;
			break;
		case WP_PROX_LAUNCHER:
			client->ps.stats[STAT_ROF] = om_proxmine_rof.integer;
			break;
	}
	...
	while ( client->timeResidual >= 1000 ) {
		...
		// Regen health
		if(om_health_regen.integer==1){
			if(ent->health<om_health_regen_maximum.integer){
				ent->health += om_health_regen_amount.integer;
				if(ent->health>om_health_regen_maximum.integer){
					ent->health = om_health_regen_maximum.integer;
				}
		...
		// Regen armor
		if(om_armor_regen.integer==1){
			if(client->ps.stats[STAT_ARMOR]<om_armor_regen_maximum.integer){
				client->ps.stats[STAT_ARMOR] += om_armor_regen_amount.integer;
				if(client->ps.stats[STAT_ARMOR]>om_armor_regen_maximum.integer){
					client->ps.stats[STAT_ARMOR] = om_armor_regen_maximum.integer;
				}
				G_AddEvent( ent, EV_POWERUP_REGEN, 0 );
			}
		}
		...
		if ( client->ps.powerups[PW_REGEN] ) {
			...
			if(om_countdown_health.integer==1){
		...
		if(om_countdown_armor.integer==1){
