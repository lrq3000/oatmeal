...
int allowedMap(char *mapname) {
    char tempStr[MAX_MAPNAME_LENGTH]; 
    int length;
    char voteMaps[MAX_CVAR_VALUE_STRING]; //= g_voteMaps.string;
    trap_Cvar_VariableStringBuffer( "g_voteMaps", voteMaps, sizeof( voteMaps ) );
    
    if(!Q_stricmp(voteMaps, "*" ))
        return qtrue; //if star, everything is allowed
    ...
    tempStr[0] = '/';
    strncpy(&tempStr[1],mapname,length);
    tempStr[length+1] = '/';
    tempStr[length+2] = '\0';
    
    if(Q_stristr(voteMaps,tempStr) != NULL) {
        return qtrue;
    }
    trap_Cvar_VariableStringBuffer( "g_voteMaps1", voteMaps, sizeof( voteMaps ) );
    if(Q_stristr(voteMaps,tempStr) != NULL) {
        return qtrue;
    }
    trap_Cvar_VariableStringBuffer( "g_voteMaps2", voteMaps, sizeof( voteMaps ) );
    if(Q_stristr(voteMaps,tempStr) != NULL) {
        return qtrue;
    }
    trap_Cvar_VariableStringBuffer( "g_voteMaps3", voteMaps, sizeof( voteMaps ) );
    if(Q_stristr(voteMaps,tempStr) != NULL) {
        return qtrue;
    }
    trap_Cvar_VariableStringBuffer( "g_voteMaps4", voteMaps, sizeof( voteMaps ) );
    if(Q_stristr(voteMaps,tempStr) != NULL) {
        return qtrue;
    }
    trap_Cvar_VariableStringBuffer( "g_voteMaps", voteMaps, sizeof( voteMaps ) );
    G_Printf("Maps on server: %s ",voteMaps);
    trap_Cvar_VariableStringBuffer( "g_voteMaps1", voteMaps, sizeof( voteMaps ) );
    G_Printf("%s ",voteMaps);
    trap_Cvar_VariableStringBuffer( "g_voteMaps2", voteMaps, sizeof( voteMaps ) );
    G_Printf("%s ",voteMaps);
...
void ThrowAmmo( gentity_t *ent ) {
	...
	if(client->ps.weapon==WP_CHAINGUN){
		if(om_chaingun_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_PROX_LAUNCHER){
		if(om_proxmine_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_ROCKET_LAUNCHER){
		if(om_rocket_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_GRENADE_LAUNCHER){
		if(om_grenade_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_BFG){
		if(om_bfg_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_PLASMAGUN){
		if(om_plasma_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_MACHINEGUN){
		if(om_machinegun_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_SHOTGUN){
		if(om_shotgun_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_LIGHTNING){
		if(om_lightning_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_RAILGUN){
		if(om_railgun_unlimited_ammo.integer==1){ return; }
	...
	if(client->ps.weapon==WP_NAILGUN){
		if(om_nailgun_unlimited_ammo.integer==1){ return; }
