...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	VectorMA (muzzle, om_gauntlet_range.integer, forward, end);
	...
	if(g_instantgib.integer) {
		...
		damage = om_gauntlet_damage.integer;
	}
...
qboolean ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
	...
	for (i = 0; i < 10; i++) {
		...
		if ( traceEnt->takedamage) {
			...
			damage = om_shotgun_damage.integer * s_quadFactor;
...
void weapon_railgun_fire (gentity_t *ent) {
	...
	damage = om_railgun_damage.integer * s_quadFactor;

	if(g_instantgib.integer)
		damage = 800;

	//VectorMA (muzzle, 8192, forward, end);
	VectorMA (muzzle, om_railgun_range.integer, forward, end);
...
//unlagged - backward reconciliation #2
	...
	} while ( unlinked < om_railgun_maximum_hits.integer );
...
void Weapon_LightningFire( gentity_t *ent ) {
	...
	damage = om_lightning_damage.integer * s_quadFactor;

	passent = ent->s.number;
	for (i = 0; i < 10; i++) {
		//VectorMA( muzzle, LIGHTNING_RANGE, forward, end );
		VectorMA( muzzle, om_lightning_range.integer, forward, end );
...
void FireWeapon( gentity_t *ent ) {
	...
	case WP_MACHINEGUN:
		...
		if ( g_gametype.integer != GT_TEAM ) {
			...
			Bullet_Fire( ent, om_machinegun_spread.integer, om_machinegun_damage.integer );
		} else {
			//Bullet_Fire( ent, MACHINEGUN_SPREAD, MACHINEGUN_TEAM_DAMAGE );
			Bullet_Fire( ent, om_machinegun_spread.integer, om_machinegun_team_damage.integer );
		}
		break;
		// oatmeal end
	...
	case WP_CHAINGUN:
		...
		Bullet_Fire( ent, om_chaingun_spread.integer, om_chaingun_damage.integer );
		// oatmeal end
		break;
	default:
